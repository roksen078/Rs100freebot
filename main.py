...
def load_settings():
    default = {
        ...
        "code": "FREE100LOOT",
        "reg_link": "https://share-rxapq9cajg.iw7.io/share/agent/SD08SPTT?data=eyJtljoxLCJsyW5kIjoxLCJpZCI6IjAifQ==",  # Default Register Link
...
@bot.message_handler(commands=['setreglink'])
def change_reg_link(message):
    if message.from_user.id != ADMIN_ID: return
    try:
        new_reg = message.text.split(maxsplit=1)[1]
        settings = load_settings()
        settings["reg_link"] = new_reg
        save_settings(settings)
        bot.reply_to(message, "✅ Broadcast 'Register Link' badal gaya bina restart kiye!")
    except:
        bot.reply_to(message, "❌ Format galat hai. Example: `/setreglink https://link.com`")
...
def admin_panel(message):
    ...
    panel_text = (
        ...
        "🎁 <code>/setcode [code]</code> - Error button code update\n"
