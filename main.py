...
def send_welcome(message):
    ...
    try: bot.send_photo(message.chif call.data == "claim_code":
        # Claim Button Click track karo
    ...
    # 2. Broadcast 'Register Link' Click Tracking Logic
    ...
    # 3. Dynamic Welcome Channels Clicks Tracking Logic
        ...
        if 0 <= btn_index < len(buttons):
            ...
            # User ko bina pata chale link khol ke dega browser me
            bot.answer_callback_query(call.id, url=buttons[btn_index]["url"])

if __name__ == "__main__":
    keep_alive()
    bot.infinity_polling()
at.id, settings.get("image"), caption=settings.get("text"), reply_markup=markup, parse_mode='HTML')
